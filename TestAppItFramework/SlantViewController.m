...
@interface SlantViewController ()

@end

@implementation SlantViewController{
    UILabel *titleLabel;
    bool isVisible;
...
- (void)viewDidLoad {
    ...
    [titleLabel setText:[LabelUtils getLabelForKey:@"slant_capture_tab"]];
...
- (float)scrollViewUI:(UIScrollView *)scrollView {
    ...
    [imageWidthLabel setHidden:true];
    ...
    [imageHeightLabel setHidden:true];
    ...
    [imageWidthTextField setHidden:true];
...
-(void)captureCameraAPICall{
    ...
    [AppItSDK captureSlantImage:[ArrayObjectController getPageDelegate] additionalDictionary:[AppItSDK getAdditionalDataDictionary] uiConfigDictionary:nsDict];
}
...
-(void)captureResponse : (NSMutableDictionary*) result {
    ...
    if(nil != front){
        ...
        NSData* data = [[NSData alloc] initWithBase64EncodedString:slant options:0];
        UIImage* image = [UIImage imageWithData:data];
        [resultImage setImage:image];
        [recaptureButton setTitle:[LabelUtils getLabelForKey:@"re_capture"] forState:UIControlStateNormal];
        isCapture=false;
